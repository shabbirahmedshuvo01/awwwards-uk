...
export default function NomineeCard({ work }: NomineeCardProps) {
  return (
      ...
      <div className="relative w-full aspect-16/10 overflow-hidden bg-[#070709]">
        ...
        <div className="absolute left-3.5 top-3.5 right-3.5 flex items-center justify-between gap-2 z-10 pointer-events-none">
          ...
          <Badge
            variant="dark"
            size="sm"
            className="pointer-events-auto text-[10px] text-zinc-300 border-white/10"
          >
            Under Review
          </Badge>
        </div>
        ...
        <div className="absolute left-3.5 bottom-3.5 z-10 pointer-events-none">
          <span className="font-mono text-[10px] uppercase tracking-wider px-2.5 py-0.5 rounded-full backdrop-blur-md border bg-black/75 border-[#0070f3]/40 text-[#38bdf8] font-semibold">
            Nominee
          </span>
        </div>
      </div>
        ...
        <div className="pt-3 border-t border-zinc-900/80 flex items-center justify-between font-mono text-[11px] text-zinc-400">
          <div className="flex flex-col">
            <Link
              ...
              className="text-zinc-200 hover:text-[#0070f3] transition-colors uppercase tracking-wider font-medium truncate max-w-40 sm:max-w-48"
            >
              {work.creator}
            </Link>
          ...
          <div className="flex items-center gap-3">
            {/* Display-only appreciation count: strictly passive, no click handler */}
            {work.appreciationCount > 0 && (
              <span
                className="text-zinc-500 text-[10px] hidden sm:inline-block"
                title={`${work.appreciationCount.toLocaleString()} appreciations`}
              >
                ♡ {work.appreciationCount.toLocaleString()}
              </span>
            )}
            <Link
              href={`/work/${work.slug}`}
