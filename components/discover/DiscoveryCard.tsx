...
export default function DiscoveryCard({ work, priority = false }: DiscoveryCardProps) {
  const aspectClasses = {
  ...
  return (
    <article className="group flex flex-col rounded-xl border border-zinc-800/80 bg-[#0c0c10] overflow-hidden transition-all duration-300 hover:border-zinc-600 shadow-sm">
      {/* Visual Container */}
      <div className={`relative w-full overflow-hidden bg-[#070709] ${aspectClasses[work.aspectRatio]}`}>
        <Link href={`/work/${work.slug}`} className="relative block h-full w-full">
          <Image
            src={work.imageUrl}
          ...
          />
          <div className="absolute inset-0 bg-linear-to-t from-black via-black/30 to-transparent opacity-85 group-hover:opacity-75 transition-opacity" />
        </Link>
        ...
        <div className="absolute left-3.5 top-3.5 right-3.5 flex items-center justify-between gap-2 z-10 pointer-events-none">
          <div className="flex items-center gap-1.5 pointer-events-auto">
            <Link href={`/categories/${work.categorySlug}`}>
              <Badge
                variant={work.recognition === 'Category Winner' ? 'accent' : 'dark'}
                size="sm"
                className="hover:opacity-90 transition-opacity text-[10px]"
              >
                {work.category}
              </Badge>
            </Link>
        ...
        <div className="absolute left-3.5 bottom-3.5 z-10 pointer-events-none">
          <span className="font-mono text-[10px] uppercase tracking-wider text-zinc-300 bg-black/65 backdrop-blur-md border border-white/10 px-2.5 py-0.5 rounded-full">
            {work.recognition}
          </span>
        </div>
      </div>
